...
function getMonths(initialMonth, numberOfMonths, withoutTransitionMonths) {
  ...
  var month = _momentJalaali(initialMonth).clone();
      ...
      function componentWillReceiveProps(nextProps) {
        ...
        if (hasMonthChanged && !hasNumberOfMonthsChanged) {
          if ((0, _isAfterDay2['default'])(initialMonth, this.props.initialMonth)) {
            ...
            newMonths.push(_momentJalaali(months[months.length - 1]).clone().add(1, 'jMonth'));
          } else {
            newMonths = months.slice(0, months.length - 1);
            newMonths.unshift(_momentJalaali(months[0]).clone().subtract(1, 'jMonth'));
          }
        }
